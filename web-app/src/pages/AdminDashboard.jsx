...
export default function AdminDashboard() {
  ...
  const handleSaveModule = async (e) => {
    e.preventDefault();
    try {
        await apiFetch(`${API_URL}/api/admin/modules`, {
            method: 'POST',
  ...
  const handleDeleteModule = async (id) => {
    ...
    await apiFetch(`${API_URL}/api/admin/modules/${id}`, { method: 'DELETE' });
    fetchData();
  };

  const handleDeleteUser = async (id) => {
    if(!window.confirm("Delete this user?")) return;
    await apiFetch(`${API_URL}/api/admin/users/${id}`, { method: 'DELETE' });
    fetchData();
  };
